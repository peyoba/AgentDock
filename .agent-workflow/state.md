...
## 当前任务
2026-07-07 AgentDock 长期会话库与终端优先布局重构 Batch 4：终端优先布局和右侧项目面板默认收起已完成并通过集成验证，准备提交后进入 Batch 5 只读项目文件树。

## 风险等级
L3

触发原因：完整目标将涉及会话持久化、PTY 生命周期、Claude/Codex 原生 resume、Renderer 主界面、文件系统读取、IPC 边界和 secret 脱敏边界。Batch 4 已完成终端优先布局、右侧项目面板默认收起和终端最小列数 CSS 约束。
...
## 下一步
提交 Batch 4 终端优先布局，然后进入 Batch 5：右侧只读项目文件树、git 状态、本会话期间变化标记和下方信息区。
